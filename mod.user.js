...
(function () {
  ...
  const iconReplacements = {
    ...
    "a[href='game.php?page=leaderboard']": {
      // Leaderboard
      icon: "ra ra-trophy",
      text: "",
    },
    "a[href='game.php?page=shop']": {
      // Buy credits
      icon: "fa-solid fa-plus",
      text: "Buy",
    },
    "session-stop[title='Stop']": {
      // Session stop
      icon: "fa-solid fa-xmark",
      text: "",
    },
    "#session-renew-btn-row2": {
      // Session renew
      icon: "fa-solid fa-arrows-rotate",
      text: "",
    },
    ".notif-bell-icon[title='Messages']": {
      // Messages
      icon: "ra ra-speech-bubble",
      text: "",
    },
    ...
    // Topbar
    "#topbar-premium>span:not([id])": {
      // Topbar Premium Badge
      icon: "ra ra-jewel-crown",
      text: "",
    },
    ...
    ".quest-tracker-icon": {
      // Quest Tracker
      icon: "ra ra-stabbed-note",
      text: "",
    },

    ".sidebar-inventory-title>span:not([class])": {
      // Inventory
      icon: "ra ra-light-backpack",
      text: " Inventory",
    },

    ".sidebar-inventory-title a[href='game.php?page=bank']": {
      // Bank
      icon: "ra ra-strongbox",
      text: " Bank",
    },

    "a[onclick*='showNewsModal']": {
      // Footer news
  ...
  function parseAndStoreSkillData(tip) {
    ...
    const requiredXPForNext = GET_XP(level - 1, "ExoToNext") || 0;
  ...
  async function EXTRACT_AMULET() {
    ...
    if (titleEl && titleEl.textContent.toLowerCase().includes("knowledge")) {
      ...
      for (const stat of stats) {
        if (stat.textContent.includes("XP Bonus")) {
          const spans = stat.querySelectorAll("span");
          if (spans.length >= 2) {
            const match = spans[1].textContent.match(/(\d+(?:\.\d+)?)/);
            if (match) {
              KX.KORUXA_GLOBALS["amulet-xp-bonus"] = Number(match[1]);
              if (typeof KX.NEH_STORAGE === "function") KX.NEH_STORAGE("save");
              break;
            }
          }
        }
  ...
  function UPDATE_DATA() {
    ...
    if (
      Object.keys(KX.KORUXA_STATS).length > 0 &&
      Object.keys(KX.KORUXA_CONFIGS).length > 0 &&
      KX.mapping["current-skill"].value !== "Doing" &&
    ...
    // Only run once when the script is ready and loaded
  ...
  function REPLACE_ICONS() {
    for (const selector in iconReplacements) {
      ...
      elements.forEach((el) => {
        ...
        if (Array.isArray(cfg.icon))
          cfg.icon.forEach((iconClass) => {
  ...
  function getXpToNext(level) {
    if (level < 1) return 0;
    return Math.floor(4 * Math.pow(level, 1.28) * Math.pow(1.074, level));
  }
  ...
  function GET_THIEVING_PROFIT_STATS(actionId) {
    ...
    if (!data) return null;
    const GoldFindBonus =
      KX.KORUXA_GLOBALS.Institute?.["GoldenTouch"]?.effect.value * KX.KORUXA_GLOBALS.Institute?.["GoldenTouch"].level || 0;
      ...
      if (results && results.length > 0) {
        ...
        if (!entry) {
          for (const cat of Object.values(config)) {
            if (cat?.[bestAction.action]) {
              entry = cat[bestAction.action];
              break;
            }
          }
        }
  ...
  function AUTO_CLAN_BOSS() {
    ...
    if (active.textContent === "⚔️ Attack Boss" && !active.hasAttribute("disabled")) {
      const planBtn = document.querySelector('#cb-plan-strip button[onclick^="selectBossTarget"]');
      const activeSpan = document.querySelector('#cb-plan-strip span[style*="#fbbf24"]');
  ...
  const INJECT_SYNC_BUTTON = () => {
    ...
    btn.style.cssText =
      "margin-left:10px; padding:4px 8px; cursor:pointer; background:var(--surface); border:1px solid var(--border); color:var(--text); border-radius:4px;";
